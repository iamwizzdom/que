...
class Paginator
{
     ...
     *
     * @param boolean $show
     *            (Optional) If set to FALSE, the "previous page" and "next page" links will only be
     ...
     * from navigation.
     * For example http://www.mywebsite.com/list will have the same content as http://www.mywebsite.com/list?page=1.
     *
     ...
     * </code>
     *
     * @param boolean $avoid_duplicate_content
     *            (Optional) If set to FALSE, the library will have for the first
     ...
     * @return void
     */
    public function baseURL($baseURL = '', $preserve_query_string = true)
    {
        // set the base URL
        $baseURL = ($baseURL == '' ? Request::getInstance()->getUri() : $baseURL);
        // parse the URL
     ...
     * <code>
     * // get the total number of pages
     * echo $pagination->getTotalRecords();
     * </code>
     *
    ...
    {
        return $this->_properties['records'];
    }
     ...
     *            Setting this argument to "left" or "right" will instruct the script to show next/previous
     ...
     *            (Optional) Setting this property to FALSE will disable padding rather than
     *            enabling it.
     *
     *            Default is TRUE.
     *
     ...
     *            The total number of records that need to be paginated
     *
     * @return void
     */
     ...
     *            (Optional) Setting this argument to TRUE will instruct the script to
     ...
     * </code>
     *
     * @param integer $selectable_pages
     *            The number of links to be displayed at once (besides the "previous page"
     *            and "next page" links).
     *
     ...
     *
     * @param boolean $enabled
     *            (Optional) Setting this property to FALSE will disable trailing slashes on generated
    ...
    public function getNextPageUrl(bool $returnUrl = false): string
    {
        ...
        if ($this->_properties['always_show_navigation'] || $this->_properties['total_pages'] > $this->_properties['selectable_pages'])
            ...
            $output = '<li class="page-item' . (($this->_properties['total_pages'] == 0 || ($this->_properties['page'] == $this->_properties['total_pages'])) ? ' disabled' : '') . '"><a class="page-link" href="' .
                ...
                ' rel="next">' . ($this->_properties['reverse'] ? $this->_properties['previous'] : $this->_properties['next']) . '</a></li>';
    ...
    private function getPages()
    {
        ...
        if ($this->_properties['total_pages'] <= $this->_properties['selectable_pages']) {
            ...
            for ($i = ($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1); ($this->_properties['reverse'] ? $i >= 1 : $i <= $this->_properties['total_pages']); ($this->_properties['reverse'] ? $i-- : $i++))
                ...
                $output .= '<li class="page-item' . ($this->_properties['page'] == $i ? ' active' : '') . '"><a class="page-link" href="' . $this->_build_uri($i) . '">' .
                    // apply padding if required
                    ($this->_properties['padding'] ? str_pad($i, strlen($this->_properties['total_pages']), '0', STR_PAD_LEFT) : $i) . '</a></li>';
            ...
            $output .= '<li class="page-item' . ($this->_properties['page'] == ($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1) ? ' active' : '') . '"><a class="page-link" href="' . $this->_build_uri($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1) . '">' .
                ...
                ($this->_properties['padding'] ?
                    ...
                    ($this->_properties['reverse'] ? $this->_properties['total_pages'] : 1)) . '</a></li>';
            ...
            $scroll_from = ($this->_properties['reverse'] ? $this->_properties['total_pages'] - ($this->_properties['selectable_pages'] - $adjacent) + 1 : $this->_properties['selectable_pages'] - $adjacent);
            ...
            if (($this->_properties['reverse'] && $this->_properties['page'] <= $scroll_from) || (!$this->_properties['reverse'] && $this->_properties['page'] >= $scroll_from)) {
                ...
                if (($this->_properties['reverse'] && $starting_page < ($this->_properties['selectable_pages'] - 1)) || (!$this->_properties['reverse'] && $this->_properties['total_pages'] - $starting_page < ($this->_properties['selectable_pages'] - 2)))
            ...
            $output .= '<li class="page-item' . ($this->_properties['page'] == $i ? ' active' : '') . '"><a class="page-link" href="' . $this->_build_uri($this->_properties['reverse'] ? 1 : $this->_properties['total_pages']) . '">' .
                // also, apply padding if necessary
                ($this->_properties['padding'] ? str_pad(($this->_properties['reverse'] ? 1 : $this->_properties['total_pages']), strlen($this->_properties['total_pages']), '0', STR_PAD_LEFT) : ($this->_properties['reverse'] ? 1 : $this->_properties['total_pages'])) . '</a></li>';
        }
        // return the resulting string
        return $output;
    }
